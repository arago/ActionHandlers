...
- Applicability:
    - Priority: 50
      ...
      Parameter:
      - Name: SshOpts
        Value: "-o StrictHostKeyChecking=no -o BatchMode=yes -o LogLevel=quiet"
  Capability:
  - Name: ExecuteCommand
    Description: "execute command on remote host"
    Command: "ssh ${SshOpts} ${User}@${Hostname} ${Command}"
    Parameter:
    - Name: Command
  ...
  - Name: RunScript
    ...
    Interpreter: "REMOTE_FILE=`basename ${TEMPFILE}`; REMOTE_FILE=\"/tmp/$REMOTE_FILE\"; scp ${SshOpts} ${TEMPFILE} ${User}@${Hostname}:$REMOTE_FILE; ssh ${SshOpts} ${User}@${Hostname} chmod u+x $REMOTE_FILE; ssh ${SshOpts} ${User}@${Hostname} $REMOTE_FILE; RC=$?; ssh ${SshOpts} ${User}@${Hostname} rm -f $REMOTE_FILE; exit $RC;"
    Command: ${Command}
    Parameter:
    - Name: Command
      Description: "command to execute"
  ...
  - Name: UploadFile
    ...
    Command: "scp -r ${SshOpts} ${Source} ${User}@${Hostname}:${Target}"
    Parameter:
    - Name: Source
  ...
  - Name: DownloadFile
    ...
    Command: 'if [ -n "${CreateTarget}" ]; then mkdir -p ${Target}; fi; scp -r ${SshOpts} ${User}@${Hostname}:${Source} ${Target}'
    Parameter:
    - Name: Source
