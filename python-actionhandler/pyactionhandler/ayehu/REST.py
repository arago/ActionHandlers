...
class RESTAPI(object):
	def __init__(self, baseurl, redis, pubsub):
		self.app=falcon.API()
		self.baseurl=baseurl
		self.redis=redis
		self.pubsub=pubsub
		self.basepath=urlparse(baseurl).path
		self.collection = CommandCollection(redis, baseurl)
		self.command = Command(self.collection)
		self.props = Property(self.command)
		self.stdout = Output(self.command, 'stdout')
		self.stderr = Output(self.command, 'stderr')
		self.cmdexit = Exit(self.command)
		self.app.add_route('{basepath}/commands'.format(basepath=self.basepath), self.collection)
		self.app.add_route('{basepath}/commands/{{id}}'.format(basepath=self.basepath), self.command)
		self.app.add_route('{basepath}/commands/{{id}}/exit'.format(basepath=self.basepath), self.cmdexit)
		self.app.add_route('{basepath}/commands/{{id}}/property/{{name}}'.format(basepath=self.basepath), self.props)
		self.app.add_route('{basepath}/commands/{{id}}/output/stdout'.format(basepath=self.basepath), self.stdout)
		self.app.add_route('{basepath}/commands/{{id}}/output/stderr'.format(basepath=self.basepath), self.stderr)
